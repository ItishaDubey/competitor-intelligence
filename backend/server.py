...
# --- SMART IMPORTS (Works from root or backend folder) ---
try:
    ...
    try:
        ...
        # Fallback dummy functions if files are missing, prevents crash
        async def scrape_url_smart(url): return {"url": url, "products": []}
        def generate_strategic_brief(a, b): return "Analysis module missing."
...
app = FastAPI(title="CI Agent", lifespan=lifespan)
...
async def register(user: UserRegister):
    ...
    res = await db.users.insert_one({
        "name": user.name, "email": email, "password": hashed_pw,
        "created_at": datetime.now(timezone.utc).isoformat()
    })
    uid = str(res.inserted_id)
    return {"access_token": create_token(uid, email), "user": {"id": uid, "name": user.name, "email": email}}
...
async def login(user: UserLogin):
    ...
    uid = str(u["_id"])
    return {"access_token": create_token(uid, email), "user": {"id": uid, "name": u["name"], "email": email}}
...
async def update_competitor(comp_id: str, comp: CompetitorUpdate, current_user: dict = Depends(get_current_user)):
    ...
    res = await db.competitors.update_one(
        {"_id": ObjectId(comp_id), "user_id": current_user["id"]}, 
        {"$set": data}
    )
...
if __name__ == "__main__":
    import uvicorn
    # 0.0.0.0 is critical for container/network access
